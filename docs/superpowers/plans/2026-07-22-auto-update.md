...
- Даёт: `ReleaseInfo` (поля `version: AppVersion`, `archiveURL: URL`, `notes: String`,
  ...
  `func latestRelease() async throws -> ReleaseInfo?`, `GitHubReleaseFetcher()`,
  `ReleaseCheckError` и `static func parse(_:) throws -> ReleaseInfo?`.
  Возврат `nil` означает «релизов нет или без ZIP-ассета» — это не ошибка.

> **Правка по итогам ревью.** В коде ниже два дефекта, исправленные при реализации; окончательный
> вариант смотрите в `Sources/PathwayCore/ReleaseInfo.swift`.
>
> 1. `keyDecodingStrategy = .convertFromSnakeCase` превращает `browser_download_url` в
>    `browserDownloadUrl` — со строчными `rl`, аббревиатуры она не распознаёт. Со свойством
>    `browserDownloadURL` ключи не совпадали, декодирование падало, `try?` глотал ошибку, и `parse`
>    возвращала `nil` **всегда**. Обновления не приходили бы никогда при зелёной сборке. Исправлено
>    явными `CodingKeys` — они видны глазом и сверяются с документацией API.
> 2. Любой не-200 статус трактовался как «релизов нет», из-за чего 403 (лимит запросов) и 5xx
>    выглядели для пользователя как «обновлений нет». Исправлено: `nil` только для 404, остальное
>    бросает `ReleaseCheckError`. По той же причине `parse` стала `throws`.
