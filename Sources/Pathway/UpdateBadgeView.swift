...
struct UpdatePopoverContent: View {
    ...
    /// Список изменений буллетами. Высота — по содержимому: короткие заметки
    /// показываются целиком, без скролла и без пустого места под последней
    /// строкой.
    ///
    /// Потолок остаётся, но не фиксированный, а от высоты экрана: заметки
    /// собраны из сообщений коммитов и на крупном релизе тянутся на десятки
    /// строк. Без потолка поповер уехал бы за край экрана вместе с кнопкой
    /// «Загрузить и установить» — обновление стало бы недоступно.
    ///
    /// `fixedSize(vertical:)` здесь обязателен: `ScrollView` по своей природе
    /// «жадный» и занимает всю предложенную высоту независимо от содержимого,
    ...
    /// Доля, а не число в точках: на 13-дюймовом ноутбуке и на внешнем мониторе
    /// «половина экрана» одинаково уместна, а фиксированные 160 pt на большом
    /// экране обрезали бы список там, где места вдоволь.
    ///
    /// Половина, а не вся высота: поповеру нужно место под шапку, кнопку и
    /// подвал, а над ним ещё строка заголовка окна.
    private static var maxNotesHeight: CGFloat {
        let screen = NSApp?.keyWindow?.screen ?? NSScreen.main
        // Запасное значение — прежние 160 pt: экран не найден только в
        // headless-окружении, где поповер всё равно никто не увидит.
        guard let height = screen?.visibleFrame.height else { return 160 }
        return height / 2
    }
