...
public final class ServerConnection {
    ...
    // MARK: - Переподключение отвалившихся томов

    /// Снимает отвалившиеся тома и монтирует их заново — все разом, после сна.
    ///
    ...
    public func reconnectStaleVolumes() async {
        // Список сверяется с системой здесь, а не поодаль: entries наполняет
        // refresh, привязанный к активации приложения, а didWake и
        // didBecomeActive приходят в непредсказуемом порядке. Опереди
        // пробуждение активацию — список был бы ещё пуст, обходить нечего, и
        // переподключение молча не делало бы ничего.
        mounted.refresh()
        ...
        for entry in entries {
            _ = await reconnectIfStale(entry.server)
        }
    }

    /// Проверяет том и, если он залип, переподключает. Возвращает точку
    /// монтирования, готовую к открытию, или nil, если тома нет и вернуть его
    /// не вышло.
    ///
    /// Нужно перед открытием вкладки сервера: залипший том выглядит
    /// подключённым, и вкладка на нём висла бы до системного таймаута. Отсюда
    /// жалоба «после сна надо руками нажать „переподключить“» — приложение
    /// считало том живым и открывало его как есть.
    ///
    /// Проба стоит только здесь и в обходе после сна, а не на каждом открытии
    /// папки: две секунды ожидания на переход в локальный каталог — заметная
    /// задержка там, где проверять нечего.
    @discardableResult
    public func reconnectIfStale(_ server: ServerAddress) async -> URL? {
        guard let point = mounted.mountPoint(for: server) else { return nil }
        guard await probe(point) == false else { return point }
        ...
        //
        // Неудача проходит молча: ноутбук, открытый дома без VPN, иначе
        // встречал бы человека алертом, хотя ничего не сломано.
        guard case .mounted(let restored) = await connect(to: server) else { return nil }
        return restored
    }
